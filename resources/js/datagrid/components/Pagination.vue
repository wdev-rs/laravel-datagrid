...
export default {
    ...
    computed: {
        showingFrom() {
            return (this.rows.currentPage - 1) * this.rows.limit + 1;
        },
        showingTo() {
            const to = this.showingFrom + this.rows.limit - 1;

            if (to > this.rows.total) {
...
<template>
    <nav class="flex items-center justify-between pt-4 pr-4" aria-label="Table navigation">
