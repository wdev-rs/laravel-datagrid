...
import Pagination from "@/Components/DataGrid/Pagination.vue";
import Actions from "@/Components/DataGrid/Actions.vue";
import SelectInput from "@/Components/SelectInput.vue";
import InputLabel from "@/Components/InputLabel.vue";
import PrimaryButton from "@/Components/PrimaryButton.vue";
import PlayIcon from "@/Components/Icons/PlayIcon.vue";
import SecondaryButton from "@/Components/SecondaryButton.vue";
import MassActions from "@/Components/DataGrid/MassActions.vue";

export default {
    components: {
        MassActions,
        SecondaryButton, PlayIcon, PrimaryButton, InputLabel, SelectInput, Actions, Pagination, Link},
    props: {
        columns: Array,
        rows: Object,
        massActions: {
    ...
    data() {
        return {
            searchKeyword: route().params.search || '',
            timer: null,
    ...
    methods: {
        ...
        {
            let key = this.rows.key;

            if (selected) {
                this.selectedIds = this.rows.data.map((row) => row[key]);
                return;
            }
    ...
    <div>
        <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                <tr v-for="(row, index) in rows.data" :key=index @click="toggleSelection(row[rows.key])"
                    class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td v-if="massActions.length" class="w-4 p-4">
                        <div class="flex items-center">
                            <input type="checkbox" :value="row[rows.key]" v-model="selectedIds"
                                   class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                            <label for="checkbox-table-search-1" class="sr-only">checkbox</label>
                        </div>
                    </td>
                    <td v-for="(column, index) in columns" class="px-4 py-4">
                        {{ row[column.id] || dot(column.id, row) }}
                    </td>
                    <td class="px-1 py-4">
                        <slot name="actions" v-bind:row="row" v-bind:key="rows.key">
                            <Actions :row="row" :primarykey="rows.key"></Actions>
                        </slot>
                    </td>
                </tr>
                </tbody>
            </table>
            <pagination :rows="this.rows"></pagination>
        </div>
    </div>
</template>
